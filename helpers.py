...
def get_all_address_data(address: str, postal: str):
    geocoder = GoogleV3(api_key=google_api_key)
    return geocoder.geocode(query=address, components={"postal_code": postal})
...
def dict_to_etree(tag_name: str, d: dict) -> etree.Element:
    ...
    def _to_etree(d, root):
        if d is None:
            ...
            root.text = etree.CDATA(f"{round(d)}")
            ...
            text = d.encode("ascii", "ignore").decode()
...
def get_restaurant(request: Request, area_code):
    ...
    db = User.query.filter_by(area_code=area_code).first()
    address = request.headers.get("X-Address")
    postal = request.headers.get("X-Postalcode")

    # Validify address and postal code
    address_data = get_all_address_data(address, postal)
    if not address_data:
        return []
